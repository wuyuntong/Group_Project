...
For the first cluster, which we label as "Rural median-sized cities with more elderly people", the first split rule is the same as the overall data, where we consider physical inactivity as the primary split, however, the second influential factor becomes the uninsuarance rate. Similar "if-then" rules could be applied in the rest of the tree. 
Significantly different tree appears to be Cluster 3 -- "small rural cities with more poverty and less education". The first determining factor becomes smoking rate, and drinking also plays an important role as it appears several times in later layers of the tree. 
