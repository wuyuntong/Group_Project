...
m1 <- lm(death_rate~., data = cl1)
m2 <- lm(death_rate~., data = cl2)
m3 <- lm(death_rate~., data = cl3)
m4 <- lm(death_rate~., data = cl4)
```
On the basis of the obtained clusters, we are able to dive into the effect of health factors on the death rate for the infection of the virus. We present a linear regression model as $death = \beta_0 + \beta_1*Smoke + \beta_2*Obs. + \beta_3*FoodEnv + \beta_4 * PhyIn + \beta_5 * Drink + \beta_6*Unins$ for the national data and clustered data. 

```{r}
varImp_plot <- function (df){
...
plot1 <- varImp_plot(varImp(m1))
plot2 <- varImp_plot(varImp(m2))
plot3 <- varImp_plot(varImp(m3))
plot4 <- varImp_plot(varImp(m4))

plot_grid(plot1, plot2, plot3, plot4, nrow = 1, align = "v")
```
