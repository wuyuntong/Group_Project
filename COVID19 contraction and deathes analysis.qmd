...
For each one of cases and deaths, we join them by the primary key countyFIPS with the rest of the pre-processed datasets. Rows with missing values are deleted. The resulting final ones are cases_f and deaths_f. These two datasets are used for clustering and regression tasks.
