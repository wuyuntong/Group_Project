...
First, our focus revolves around understanding the dynamics of the pandemic's progression within the US. It is crucial to delve into the data and find out whether there are tangible variations in the trends at some critical moments. For instance, we want to see if there is a dramatic increase in the confirmed cases or deaths during the outbreak of a new virus variant and how these numbers evolve as time goes by.
...
cov_clean <- function(df) {
  ...
  df <- df |>
    select(
      countyFIPS, `County Name`, State, StateFIPS, `2020-06-30`,
...
```{r}
p_smoke <- ggplot(data = cases_f, aes(x = Smoke)) +
...
plot_grid(p_smoke, p_obes, p_food, p_phys, p_drink, p_unins, nrow = 2, ncol = 3, align = "v")
...
Next we aim to identify the specific trends in cases and deaths across various regions in the United States over a certain period. We have selected the daily cumulative data from June 31, 2020, to June 30, 2021, and used the ARIMA model for analysis.

#### Model introduction

ARIMA (Auto-Regressive Integrated Moving Average) is a popular approach to modelling dynamical systems. ARIMA model uses the difference method to eliminate trend and seasonality. The resulting series is treated as stationary time series data and is modelled using combination of its lagged time series values (“AR”) and moving average of lagged forecast errors (“MA”). An ARIMA model is typically specified by a tuple (p, d, q), where p and q define the orders of AR and MA, and d specifies the order of differencing operation.

The advantage of the ARIMA model is that the model is simple and only requires endogenous variables without the help of other exogenous variables. Nevertheless, ARIMA model also has disadvantages: The time series data is required to be stable, or be stable after differentiation; essentially it can only capture linear relationships, not non-linear relationships.

$(1 - \sum_{i=1}^{p} \phi_{i} L^{i} ) \; (1-L)^{d} \; X_{t} = (1 + \sum_{i = 1}^{q} \theta_{i}L^{i}) \;\epsilon_{t}$

#### Empirical analysis

In order to test the accuracy of model fitting, we divided the data set into a training set and a test set. The initial 80% of the data is used as the training set, and the remaining 20% is utilized as the test set.

##### National cases
Initially, by plotting graphs and conducting an ADF test, we found that the original data was not stationary. After performing a second differencing, we obtained a relatively stable dataset.
...
By applying the ADF test on the second-order difference data, we find the test statistic is `r adf1$statistic` with a lag order of 6, resulting in a p-value of `r adf1$p.value`. We thus reject the null hypothesis (under the 5% significance level), and the stationarity is supported for the second-order differenced series.

By minimizing AIC, the obtained ARIMA model is an ARIMA(2,2,4) with the estimated cofficients $\phi_{1} = 0.7446, \phi_{2} = -0.6193,$ $\phi_{3} = -0.0969, \phi_{4} = -0.2473, \theta_{1} = -1.0541 \; \text{and} \; \phi_{2} = 0.6374$. By inspecting the residuals, we find it tends to be a white noise, although the residuals do not seem to be normal.

We then use the fitted model to predict the number of cases and compare it with the test set. As shown in the figure below, the blue line is the model prediction curve, the red line is the curve of the test set, and the shaded area is the confidence interval. The model prediction curve is not much different from the curve of the test set, and both are within the confidence interval.
...
plot(forecast(model_case, h = floor(0.3 * length(case_ts)) + 1) )
lines(test_case, lwd = 2, col = "red") 
```

##### National deaths

Next, we perform a similar analysis on national deaths data. From the results, we can see that the death data becomes stable after second-order difference. By minimizing AIC, we obtain  an ARIMA(5,2,2). The model prediction curve (blue line) and the test set curve (red line) are within the confidence interval, which means that we have reason to believe that this model reflects the information of the data to a certain extent. 
...
plot(forecast(model_death, h = floor(0.3 * length(case_ts))+ 1))
...
## Conclusion
This report investigates into the COVID-19 infection and deaths across the counties in US from June 2020 and July. We refer to the two-stage methodology as outlined in de Ona (2016). First, we cluster the counties using K-means clustering based on the their social economic profile, from which we are able to identify four distinct clusters. Then we build regression trees for each of the clusters to study the relationship between the death rate and health-related factors, which enables us to better target specific regions to reduce the COVID caused deaths. The findings mostly agree on that physical inactivity being the foremost important factors in COVID caused deaths. The exception is the rural areas with high poverty and low education, where uninsuarance becomes the most important and drinking and smoking also are deemed important. This finding could be used for policy makers to better

Given the amount of missing values in the data and not considering the entries with these missing values, our analysis is conducted on the basis of `r nrow(cases_f)` counties out of `r nrow(cases)`. We must acknowledge that there could exist undiscovered patterns if we had access to those values and this also has some effect on the regression tree analysis. Secondly, we determine the number of clusters simply by elbow rule, which . Thirdly, we also have to point that the trees are not a stable method -- a small variation in the data could result in a different tree. Ensemble and boosting methods, such as random forests and gradient boosted trees, could be utilized to enhance the accuracy and stability but at the cost of the loss of easy interpretability of trees.
